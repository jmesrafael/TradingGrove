...
(async () => {
  ...
  if (justUpgraded) {
    // Record the pro plan on this device so the dashboard's free->pro
    // transition check doesn't fire a second celebration.
    try { localStorage.setItem('tz_last_plan_' + user.id, 'pro'); } catch (e) {}
