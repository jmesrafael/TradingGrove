...
let _themeDropOpen=false,_fontDropOpen=false,_menuOpen=false,_notifOpen=false,_notifs=[];
...
document.addEventListener('click',e=>{
  ...
  if(_notifOpen){const w=document.getElementById('notifWrap');if(w&&!w.contains(e.target))closeNotifMenu();}
...
// ── Notifications (admin -> user messages) ───────────────────────────────────
function toggleNotifMenu(){_notifOpen=!_notifOpen;document.getElementById('notifMenu').classList.toggle('open',_notifOpen);if(_notifOpen)closeMenu();}
function closeNotifMenu(){_notifOpen=false;document.getElementById('notifMenu')?.classList.remove('open');}
...
function renderNotifs(){
  ...
  document.getElementById('notifMarkAll').style.display=unread?'':'none';
  if(!_notifs.length){list.innerHTML='<div class="notif-empty">No notifications yet.</div>';return;}
  list.innerHTML=_notifs.map(n=>`
    <div class="notif-item${n.read_at?'':' unread'}" onclick="notifClick('${n.id}')">
      <span class="notif-dot"></span>
      <div style="min-width:0;flex:1">
        <div class="notif-title">${esc(n.title)}</div>
        <div class="notif-body">${esc(n.body)}</div>
        <div class="notif-time">${_notifTime(n.created_at)}</div>
      </div>
    </div>`).join('');
}
...
function markAllNotifsRead(){
  ...
  _notifs.forEach(n=>{if(!n.read_at)n.read_at=now;});
  renderNotifs();
  markAllNotificationsRead(currentUser.id);
}
async function initNotifications(){
  try{
    _notifs=await getNotifications(currentUser.id);
    renderNotifs();
    subscribeNotifications(currentUser.id,payload=>{
      if(!payload?.new)return;
      _notifs.unshift(payload.new);
      renderNotifs();
      showToast(payload.new.title,'fa-solid fa-bell');
    });
  }catch(e){console.warn('[dashboard] notifications unavailable:',e);}
}
...
(async()=>{
  ...
  // Pro celebration: fire once per device on any free -> pro transition
  // (PayPal, admin grant, referral reward). Absent key = first visit on this
  // device; store silently so an already-pro user never gets a false fire.
  try{
    const planKey='tz_last_plan_'+currentUser.id;
    const lastPlan=localStorage.getItem(planKey);
    if(lastPlan==='free'&&isPro)setTimeout(openProCelebration,700);
    localStorage.setItem(planKey,isPro?'pro':'free');
  }catch(e){}
...
// ── Pro celebration modal (ported from subscription.js reward modal) ─────────
function openProCelebration(){
...
function closeRewardModal(){
  ...
  setTimeout(()=>{overlay.classList.remove('open');overlay.style.opacity='';overlay.style.transition='';},260);
}
document.getElementById('rewardOverlay').addEventListener('click',function(e){if(e.target===this)closeRewardModal();});
document.addEventListener('keydown',function(e){if(e.key==='Escape')closeRewardModal();});
